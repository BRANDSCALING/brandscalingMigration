...
import { ArrowRight, ArrowLeft, Brain, Heart, Clock } from "lucide-react";
...
export function Step4_TimingInput({ onNext, onBack, initialData }: Step4Props) {
  ...
  const submitMutation = useMutation({
    mutationFn: async (data: Step4Data) => {
      return apiRequest("POST", "/api/wizard/step4", {
    ...
    onSuccess: (result) => {
      toast({
        title: "Timing analysis captured!",
        description: `Your ${dnaType.toLowerCase()} timing insight has been saved.`,
      });
      if (onNext) {
    ...
    onError: (error) => {
      toast({
        title: "Error saving timing analysis",
  ...
  const handleNextStep = () => {
    if (!response.trim()) {
      toast({
        title: "Please share your timing insight",
        description: "Enter your timing analysis before proceeding to the next step.",
        variant: "destructive",
      });
      return;
    }
  ...
  const prompts = {
    Architect: "Why now? What makes this urgent?",
    Alchemist: "Why does this matter now in your life or the world?"
  };
      ...
      <Card className="w-full max-w-2xl p-8 shadow-xl border-0 rounded-3xl">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center px-4 py-2 bg-gray-100 rounded-full mb-4">
            <span className="text-gray-600 text-sm font-medium">Step 4 of 10</span>
          </div>
          ...
          <p className="text-gray-600">
            Understanding why this matters right now
          </p>
        </div>
        ...
        <div className={`p-6 rounded-2xl mb-6 ${colors.accent}`}>
          ...
          {dnaType === "Architect" && (
            <p className="text-gray-600 text-sm mt-2">
              Consider market timing, competitive landscape, technological readiness, and external pressures.
            </p>
          )}
          {dnaType === "Alchemist" && (
            <p className="text-gray-600 text-sm mt-2">
              Think about personal calling, societal shifts, generational needs, and the moment of transformation.
            </p>
          )}
        </div>
        ...
        <div className="mb-8">
          ...
          <Textarea
            ...
            placeholder={dnaType === "Architect" 
              ? "What market conditions, trends, or competitive gaps make this the right time? Is there urgency from regulations, technology, or consumer behavior shifts?"
              : "What's happening in your life or the world that makes this feel urgent now? What deeper calling or moment of clarity is driving this timing?"
            }

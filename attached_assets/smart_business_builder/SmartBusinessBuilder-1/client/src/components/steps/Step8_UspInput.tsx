...
import { ArrowRight, ArrowLeft, Brain, Heart, Star } from "lucide-react";
...
export function Step8_UspInput({ onNext, onBack, initialData }: Step8Props) {
  ...
  const submitMutation = useMutation({
    mutationFn: async (data: Step8Data) => {
      return apiRequest("POST", "/api/wizard/step8", {
    ...
    onSuccess: (result) => {
      toast({
        title: "Unique value proposition captured!",
        description: `Your ${dnaType.toLowerCase()} USP has been saved.`,
      });
      if (onNext) {
  ...
  const handleNextStep = () => {
    if (!response.trim()) {
      toast({
        title: "Please describe your unique value",
        description: "Enter what makes your offer different before proceeding to the next step.",
        variant: "destructive",
      });
      return;
    }
  ...
  const prompts = {
    Architect: "What makes this different or better?",
    Alchemist: "Why would someone choose you instead of others?"
  };
      ...
      <Card className="w-full max-w-2xl p-8 shadow-xl border-0 rounded-3xl">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center px-4 py-2 bg-gray-100 rounded-full mb-4">
            <span className="text-gray-600 text-sm font-medium">Step 8 of 10</span>
          </div>
        ...
        <div className={`p-6 rounded-2xl mb-6 ${colors.accent}`}>
          ...
          {dnaType === "Architect" && (
            <p className="text-gray-600 text-sm mt-2">
              Focus on competitive advantages, unique features, superior quality, innovative methods, or measurable benefits.
            </p>
          )}
          {dnaType === "Alchemist" && (
            <p className="text-gray-600 text-sm mt-2">
              Think about your personal touch, authentic connection, deeper purpose, and the transformation you uniquely provide.
            </p>
          )}
        </div>
        ...
        <div className="mb-8">
          ...
          <Textarea
            ...
            placeholder={dnaType === "Architect" 
              ? "What specific advantages do you offer? Better quality, faster delivery, lower cost, unique technology, proven results? What sets your solution apart from competitors?"
              : "What makes your approach special? How do you connect with people differently? What unique perspective, story, or transformation do you bring that others can't?"
            }

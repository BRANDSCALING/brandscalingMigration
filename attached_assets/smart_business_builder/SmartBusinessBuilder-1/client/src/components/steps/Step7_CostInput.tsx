...
import { ArrowRight, ArrowLeft, Brain, Heart, Calculator } from "lucide-react";
...
export function Step7_CostInput({ onNext, onBack, initialData }: Step7Props) {
  ...
  const submitMutation = useMutation({
    mutationFn: async (data: Step7Data) => {
      return apiRequest("POST", "/api/wizard/step7", {
    ...
    onSuccess: (result) => {
      toast({
        title: "Cost analysis captured!",
        description: `Your ${dnaType.toLowerCase()} cost assessment has been saved.`,
      });
      if (onNext) {
    ...
    onError: (error) => {
      toast({
        title: "Error saving cost analysis",
  ...
  const handleNextStep = () => {
    if (!response.trim()) {
      toast({
        title: "Please describe your costs",
        description: "Enter your cost and effort analysis before proceeding to the next step.",
        variant: "destructive",
      });
      return;
    }
  ...
  const prompts = {
    Architect: "What does it cost you to deliver this?",
    Alchemist: "What effort, time, or resources does this take from you?"
  };
      ...
      <Card className="w-full max-w-2xl p-8 shadow-xl border-0 rounded-3xl">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center px-4 py-2 bg-gray-100 rounded-full mb-4">
            <span className="text-gray-600 text-sm font-medium">Step 7 of 10</span>
          </div>
          <div className="flex items-center justify-center mb-4">
            <Calculator className="w-8 h-8 text-gray-600 mr-3" />
            <h1 className="text-3xl font-bold text-gray-900">
              Cost & Effort Analysis
            </h1>
          </div>
          <p className="text-gray-600">
            Understand what it takes to deliver your offer
          </p>
        </div>
        ...
        <div className={`p-6 rounded-2xl mb-6 ${colors.accent}`}>
          ...
          {dnaType === "Architect" && (
            <p className="text-gray-600 text-sm mt-2">
              Consider material costs, time investment, overhead expenses, tools, software, and operational costs.
            </p>
          )}
          {dnaType === "Alchemist" && (
            <p className="text-gray-600 text-sm mt-2">
              Think about your energy expenditure, emotional investment, creative resources, and personal sacrifices required.
            </p>
          )}
        </div>
        ...
        <div className="mb-8">
          ...
          <Textarea
            ...
            placeholder={dnaType === "Architect" 
              ? "Break down your costs: materials, time, tools, overhead, outsourcing, etc. What's your cost per unit or project? What are your fixed vs. variable expenses?"
              : "What does this work take from you personally? How much energy, creativity, and emotional investment? What do you give up or sacrifice to deliver this?"
            }

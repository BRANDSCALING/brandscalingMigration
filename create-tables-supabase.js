...
const connectionString = 'postgresql://postgres:Supabasereplitpass@1@db.mhygcdbfvwceiskerzod.supabase.co:5432/postgres';
...
async function createSupabaseTables() {
  ...
  try {
    ...
    // Verify data
    const testConnectionCount = await client.query('SELECT COUNT(*) as count FROM test_connection');
    const workbookCount = await client.query('SELECT COUNT(*) as count FROM workbook_sessions');
    
    console.log(`✅ Verification complete:`);
    console.log(`  - test_connection table: ${testConnectionCount.rows[0].count} records`);
    console.log(`  - workbook_sessions table: ${workbookCount.rows[0].count} records`);

    // Show sample data
    const sampleTest = await client.query('SELECT name, email FROM test_connection LIMIT 2');
    const sampleWorkbook = await client.query('SELECT user_id, dna_mode, user_email FROM workbook_sessions LIMIT 2');
    ...
    sampleWorkbook.rows.forEach(row => console.log(`  - ${row.user_id} (${row.dna_mode}) - ${row.user_email}`));

    console.log('\n🎉 SUCCESS! Tables created in your Supabase database!');
    console.log('Go to your Supabase Table Editor and refresh - you should now see:');
    ...
    if (error.message.includes('ENOTFOUND')) {
      ...
      console.log('1. Check if the hostname is correct: db.mhygcdbfvwceiskerzod.supabase.co');
      console.log('2. Verify your Supabase project is active');
      console.log('3. Try using the connection string directly from Supabase dashboard');
    }
  } finally {
    if (client) {
      await client.end();
